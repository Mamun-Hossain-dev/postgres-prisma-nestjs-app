...
export function CartPage() {
  ...
  const clear = useMutation({
    mutationFn: () =>
      apiFetch<Cart>('/cart', { method: 'DELETE' }, accessToken),
    onSuccess: (data) => {
  ...
  if (!user) {
    return (
      <div className="mx-auto flex min-h-[70vh] max-w-xl flex-col items-center justify-center px-5 text-center">
        ...
        <Link
          href="/login"
          className="mt-8 rounded-full bg-ink px-7 py-4 font-bold text-white"
        >
          Sign in
        </Link>
      </div>
  ...
  if (cart.isError) {
    return (
      <div className="mx-auto min-h-[70vh] max-w-3xl px-5 py-16">
        <EmptyState
          icon={<ShoppingBag />}
          title="Your cart could not be loaded"
          description={cart.error.message}
          action={
            <Button onClick={() => void cart.refetch()}>Try again</Button>
          }
        />
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-16 lg:px-8">
      ...
      <ConfirmDialog
        ...
        description="Every product currently in your cart will be removed."
